...
svp::PrimeMetaData svp::HybridMpiOpenMP_PrimeMetaDataStrategy::calculateBiggestPrime(const uint32_t limit) {
    SVP_PROFILE_SCOPE(toString().c_str());
    ...
    #pragma omp parallel for schedule(dynamic) default(none) firstprivate(start, limit, stride) shared(largestPrime, primeCount)
    for(uint32_t no = start; no <= limit; no+=stride) {
        bool primeFound = true;
        for(uint32_t divisor = 3; divisor*divisor <= no; divisor+=2) {
            if(no%divisor == 0) {
        ...
        if(primeFound) {
            #pragma omp critical
            {
                largestPrime = std::max(largestPrime, no);
                primeCount++;
            }
        }
    }
...
std::string svp::HybridMpiOpenMP_PrimeMetaDataStrategy::toString() {
    return "Get #prime and the largest prime <= Limit using MPI + OpenMP";
}
